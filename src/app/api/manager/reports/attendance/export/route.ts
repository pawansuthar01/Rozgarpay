...
export async function GET(request: Request) {
  try {
    ...
    const attendanceData = await prisma.attendance.findMany({
      ...
      orderBy: [{ attendanceDate: "asc" }, { user: { firstName: "asc" } }],
    });
    ...
    return new NextResponse(JSON.stringify(exportData, null, 2), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="attendance-report-${dateFrom}-to-${dateTo}.json"`,
      },
    });
  } catch (error) {
    console.error("Manager attendance export error:", error);
    return NextResponse.json(
